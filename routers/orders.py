...
async def analyze_order(request: PromptRequest):
    ...
    **Instrucciones de extracción y auditoría:**
    ...
    5. **Auditoría (Discrepancias):** Revisa el 'Historial Reciente'. Compara lo que pidió el cliente con lo que el admin anotó en 'Texto del Pedido'. Si hay una discrepancia clara e involuntaria (ej. cliente pidió "premium" y admin anotó "clásico"), marca "detectada": true y explica en "motivo".
    6. **Autoridad del Admin (Excepciones):** Si el administrador está insistiendo con un pedido (repite el mismo resumen), si está corrigiendo un error previo, o si asume conscientemente el control del pedido, DEBES confiar en el admin y marcar "detectada": false. El administrador es el jefe y tiene la última palabra; no lo contradigas si está claro que su acción es intencional.
